...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Experiments\\tid_psam\\psam\\untitled_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "test_laptop" ---
    ...
    while continueRoutine and routineTimer.getTime() < 2.0:
        ...
        if mic.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > mic.tStartRefresh + 2.0-frameTolerance:
                # keep track of stop time/frame for later
    ...
    # --- Prepare to start Routine "test_laptop" ---
    # create an object to store info about Routine test_laptop
    test_laptop = data.Routine(
        name='test_laptop',
        components=[text_2],
    )
    test_laptop.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for test_laptop
    test_laptop.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    test_laptop.tStart = globalClock.getTime(format='float')
    test_laptop.status = STARTED
    thisExp.addData('test_laptop.started', test_laptop.tStart)
    test_laptop.maxDuration = None
    # keep track of which components have finished
    test_laptopComponents = test_laptop.components
    for thisComponent in test_laptop.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "test_laptop" ---
    test_laptop.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine and routineTimer.getTime() < 1.0:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            test_laptop.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in test_laptop.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "test_laptop" ---
    for thisComponent in test_laptop.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for test_laptop
    test_laptop.tStop = globalClock.getTime(format='float')
    test_laptop.tStopRefresh = tThisFlipGlobal
    thisExp.addData('test_laptop.stopped', test_laptop.tStop)
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if test_laptop.maxDurationReached:
        routineTimer.addTime(-test_laptop.maxDuration)
    elif test_laptop.forceEnded:
        routineTimer.reset()
    else:
        routineTimer.addTime(-1.000000)
    thisExp.nextEntry()
