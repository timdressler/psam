...
expName = 'tid_psam_audiotest1'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Experiments\\tid_psam\\psam\\testing\\tid_psam_audiotest1.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisTrial in trials:
        ...
        sound_2.play(when=curt+0.5)
        
        # p_port.setData(4)
        sound_1.setSound('A', secs=0, hamming=False)
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            if p_port.status == STARTED:
                ...
                if tThisFlipGlobal > p_port.tStartRefresh + 0.05-frameTolerance:
            ...
            if sound_1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > sound_1.tStartRefresh + 0-frameTolerance or sound_1.isFinished:
                    # keep track of stop time/frame for later
