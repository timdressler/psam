...
fal_promptTexts = containers.Map(...
    ...
    'var2_handedness: (Angeben: 1 = rechtshändig, 2 = linkshändig, 3 = beidhändig)', ...
    'var3_sex: (Angeben: 1 = männlich, 2 = weiblich, 3 = divers)', ...
...
if fal_skipped ~= 1
    ...
    for i = 2:width(fal)
        ...
        elseif strcmp(newRow{i-1},'1') % Only ask for detail if needed
            while true
                answer = inputdlg(fal_promptTexts(varName), 'FAL', [1 70]);
                if isempty(answer)
                    disp('Cancelled.');
